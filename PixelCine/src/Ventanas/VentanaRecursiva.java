	...
	public JFramePrincipal(List<Pelicula> Peliculas) {
		...
		KeyListener myKeyListener = new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				// CTRL + C - CreaciÃ³n de un nuevo Pelicula
				if (e.getKeyCode() == KeyEvent.VK_C && e.isControlDown()) {
					...
					if (result == JOptionPane.OK_OPTION) {
						if (!txtTitulo.getText().isEmpty()) {
							Pelicula Pelicula = new Pelicula(Peliculas.size()+1, (Editorial) jcomoEditorial.getSelectedItem(), txtTitulo.getText());
							Peliculas.add(Pelicula);
							//Se borra el filtro
							txtFiltro.setText("");
							//Se cargan de nuevo los Peliculas
							loadPeliculas();
						} else {
							JOptionPane.showMessageDialog(null, "El tÃ­tulo no puede estar vacÃ­o", "Error", JOptionPane.ERROR_MESSAGE);
						}						
					}
				} else if (tablaPeliculas.getSelectedRow() != -1 && e.getKeyCode() == KeyEvent.VK_P && e.isControlDown()) {					
					Pelicula Pelicula = Peliculas.get((int) tablaPeliculas.getValueAt(tablaPeliculas.getSelectedRow(), 0) - 1);
					List<Actor> Actors = new ArrayList<>();
					
					//Se recorren todos los Peliculas
					Peliculas.forEach(c -> {
						//Se recorren todos los Actors del Pelicula
						c.getActors().forEach(p -> {
							//Se aÃ±aden a la lista de Actors sin repeticiÃ³n
							if (!Actors.contains(p) && !Pelicula.getActors().contains(p)) {
								Actors.add(p);
							}
						});		
					});					
					
					//Se crea el cuadro de diÃ¡logo de modificaciÃ³n de Actors.
					new JDialogActors(Actors, Pelicula);
					
					//Se borra el filtro
					txtFiltro.setText("");
					//Se cargan de nuevo los Peliculas para actualizar el nÃºmero de Actors
					loadPeliculas();
										
					//Se carga de nuevo la tabla de Actors
					loadActors(Pelicula);
				}
			}
		...
		this.tablaActors.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseExited(MouseEvent e) {
				//Se resetea la fila/columna sobre la que estÃ¡ el ratÃ³n				
				mouseRowActors = -1;
			}
		});
