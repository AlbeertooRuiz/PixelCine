...
public class VentanaRecursiva extends JFrame {
	...
	public void JFramePrincipal(List<Pelicula> Peliculas) {
		...
		// Se define el listener para detectar cuando el ratÃ³n sale de la tabla de
		// Actors

		// Se define el listener para controlar el movimiento del ratÃ³n sobre la tabla
		// de Actors
	...
	private void initTables() {
		...
		Vector<String> cabeceraPeliculas = new Vector<String>(
				Arrays.asList("Nombre", "Duracion", "Categoria", "Actor Principal"));
		...
		// Se define un CellRenderer para las celdas de las dos tabla usando una
		// expresiÃ³n lambda
		TableCellRenderer cellRenderer = (table, value, isSelected, hasFocus, row, column) -> {
			...
			if (value instanceof Categoria) {
				...
				try {
					...
					if (table.equals(tablaPeliculas)) {
						...
						if (isSelected) {
							txtResaltado = "<strong style='background-color:white; color: red;'>" + filter
									+ "</strong>";
						} else {
							txtResaltado = "<strong style='background-color:yellow; color: blue;'>" + filter
									+ "</strong>";
						}
						...
						txtHtml.append(
								txtValue.substring(txtValue.indexOf(filter) + filter.length(), txtValue.length()));
						txtHtml.append("</html");
			...
			// La filas pares e impares se renderizan de colores diferentes de la tabla de
			// Peliculas
			if (table.equals(tablaPeliculas)) {
				if (row % 2 == 0) {
			...
			// Si la celda estÃ¡ seleccionada se renderiza con el color de selecciÃ³n por
			// defecto
		...
		// Se define un CellRenderer para las cabeceras de las dos tabla usando una
		// expresiÃ³n lambda
		...
		// Se modifica el modelo de selecciÃ³n de la tabla para que se pueda selecciona
		// Ãºnicamente una fila
		...
		this.tablaPeliculas.getSelectionModel().addListSelectionListener(e -> {
			...
			if (tablaPeliculas.getSelectedRow() != -1) {
				this.loadActors(
						this.Peliculas.get((int) tablaPeliculas.getValueAt(tablaPeliculas.getSelectedRow(), 0) - 1));
			}
		});
	...
	private void filtrarPeliculas() {
		...
		PeliculasOrdenados.forEach(c -> {
			if (c.getNombre().contains(this.txtFiltro.getText())) {
				this.modeloDatosPeliculas.addRow(new Object[] { c.getNombre(), c.getFechayhora(), c.getDuracion(),
						c.getCategoria(), c.getAsientosDisponibles(), c.getActorPrincipal() });
			}
		});
	}
	...
	private void loadPeliculas() {
		...
		PeliculasOrdenados
				.forEach(c -> this.modeloDatosPeliculas.addRow(new Object[] { c.getNombre(), c.getFechayhora(),
						c.getDuracion(), c.getCategoria(), c.getAsientosDisponibles(), c.getActorPrincipal() }));
	}
	...
	private void loadActors(Pelicula Pelicula) {
		...
		/*
		 * Pelicula.getActorPrincipal().forEach(p -> this.modeloDatosActors.addRow( new
		 * Object[]{c.getNombre(), c.getFechayhora(), c.getDuracion(), c.getCategoria(),
		 * c.getAsientosDisponibles()}) );
		 */
		// Se modifica el texto del bode de la lista de Actors
		this.scrollPaneActors.setBorder(
				new TitledBorder(String.format("Actors del Pelicula '%s' [%d]", Pelicula.getActorPrincipal())));
	}
}